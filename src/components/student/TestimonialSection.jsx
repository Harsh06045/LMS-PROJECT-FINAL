...
const TestimonialSection = () => {
  ...
  return (
    <div className={`pb-14 px-8 md:px-0 transition-colors duration-300 ${
      isDarkMode ? 'bg-gray-900' : ''
    }`}>
      {/* Header Section */}
      <h2 className={`text-3xl font-medium transition-colors duration-300 ${
        isDarkMode ? 'text-white' : 'text-gray-800'
      }`}>
        Testimonials
      </h2>
      <p className={`md:text-base mt-3 transition-colors duration-300 ${
      ...
      <div className='grid grid-cols-auto gap-8 mt-14'>
        {dummyTestimonial.map((testimonial, index) => (
          ...
          >
            {/* User Info Header */}
            <div className={`flex items-center gap-4 px-5 py-4 transition-colors duration-300 ${
              isDarkMode ? 'bg-gray-700/50' : 'bg-gray-500/50'
            }`}>
              <img 
